...
// ========== ALUMNOS ==========
// Función auxiliar para normalizar grupoActual
...
// ========== TAREAS (CRUD y listado por clase) ==========
...
// ========== RUTAS DE CLASES PARA ALUMNO Y DOCENTE ==========
...
// ========== RUTA PARA OBTENER UNA CLASE POR ID (DOCENTE) ==========
...
// ========== RUTAS PARA ESTUDIANTE (desde admin, pero se pueden separar) ==========
// Obtener todas las entregas de un estudiante por su matrícula
router.get('/estudiante/entregas', async (req, res) => {
    try {
        const { matricula } = req.query;
        if (!matricula) {
...
// Guardar lista completa de horarios (reemplaza todos)
// Guardar lista completa de horarios (reemplaza todos)
router.post('/alumnos/:matricula/horario-personal', async (req, res) => {
    try {
        ...
        // Validar que cada horario tenga los campos necesarios
        for (const h of horarios) {
            if (!h.dia || !h.hora || !h.materia) {
                return res.status(400).json({ error: 'Cada horario debe tener día, hora y materia' });
            }
        }
        // Eliminar todos los horarios anteriores del alumno
        await HorarioAlumno.deleteMany({ alumnoMatricula: matricula });
        // Insertar los nuevos, pero sin el campo _id (para que MongoDB genere uno nuevo)
        const nuevos = horarios.map(h => {
            const { _id, ...resto } = h;   // elimina _id si existe
            return { ...resto, alumnoMatricula: matricula };
        });
        let inserted = [];
        if (nuevos.length > 0) {
        ...
        console.error('Error POST horario personal:', err);
        res.status(500).json({ error: err.message });
    }
});

// Eliminar una entrada específica (opcional, si se quiere borrar individualmente)
router.delete('/alumnos/:matricula/horario-personal/:id', async (req, res) => {
    try {
        const { id } = req.params;
        await HorarioAlumno.findByIdAndDelete(id);
...
// ========== BOLETAS (PDF) ==========
// Configuración de multer para boletas
...
// Subir boleta (desde panel de administrador)
...
// ========== CAMBIAR CONTRASEÑA DE ALUMNO (desde su perfil) ==========
router.put('/alumno/password', async (req, res) => {
    try {
        ...
        // Validar la nueva contraseña (máx 8 caracteres, un número y un símbolo)
...
// Listar boletas de un alumno (para el estudiante)
...
// Eliminar boleta (opcional, para admin)
